...
void Tileset::setTileSize(const mathgp::uvector2& size)
{
    assert(m_texture);
    ...
    for (int height = m_margin; height < texh - m_margin; height += m_spacing + size.y())
    {
        ...
        for (int width = m_margin; width < texw - m_margin; width += m_spacing + size.x())
        {
            ...
            sf.tc._01 = vc(left, bottom);
            sf.tc._11 = vc(right, bottom);
            sf.tc._00 = vc(left, top);
            sf.tc._10 = vc(right, top);
...
// not implemented yet
//void Tileset::setTilesPerSide(mathgp::uvector2& tilesPerSide)
//{
//
//}
